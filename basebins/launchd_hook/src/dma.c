...
static pthread_mutex_t __dma_lock = PTHREAD_MUTEX_INITIALIZER;

static void dma_lock(void) {
    usleep(80);
    pthread_mutex_lock(&__dma_lock);
    usleep(1000);
}

static void dma_unlock(void) {
    usleep(80);
    pthread_mutex_unlock(&__dma_lock);
    usleep(1000);
}
...
void dma_deinit(void) {
    ...
    if (dma_info == NULL) return;
    if (dma_info->mapping_list != NULL) {
        for (uint32_t i = 0; i < dma_info->mapping_count; i++) {
            if (dma_info->mapping_list[i].va != 0) {
                mach_vm_deallocate(mach_task_self(), dma_info->mapping_list[i].va, kinfo->page_size);
                dma_info->mapping_list[i].va = 0;
            }
            dma_info->mapping_list[i].pa = 0;
        }
        free(dma_info->mapping_list);
    }

    if (dma_info->cacheline_data_va != 0) {
        mach_vm_deallocate(mach_task_self(), dma_info->cacheline_data_va, kinfo->page_size);
    }

    bzero(dma_info, sizeof(dma_info_t));
    free(dma_info);
    dma_info = NULL;
    return;
}
...
int dma_init(void) {
    ...
    if ((dma_info->mapping_list = calloc(1, sizeof(dma_mapping_t) * 20)) == NULL) goto err;
    dma_info->mapping_count = 20;
    
    uint32_t flags = VM_FLAGS_ANYWHERE | VM_FLAGS_NO_CACHE;
    mach_vm_allocate(mach_task_self(), &dma_info->cacheline_data_va, kinfo->page_size, flags);
    if (dma_info->cacheline_data_va == 0) goto err;
    
    mach_vm_wire(mach_host_self(), mach_task_self(), dma_info->cacheline_data_va, kinfo->page_size, VM_PROT_DEFAULT);    
    bzero((void *)dma_info->cacheline_data_va, kinfo->page_size);
    
    if ((dma_info->cacheline_data_pa = vtophys(dma_info->cacheline_data_va)) == 0) {
        asm volatile ("dsb sy");
        usleep(10000);
        if ((dma_info->cacheline_data_pa = vtophys(dma_info->cacheline_data_va)) == 0) goto err;
    }

    dma_info->cacheline_data = (uint8_t *)dma_info->cacheline_data_va;
